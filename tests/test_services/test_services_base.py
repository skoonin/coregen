...
class TestServicesBase:
    ...
    def test_apply_filters(self, mock_config_provider):
        """Test apply_filters method delegates to FilterService."""
        # Use mock_config_provider fixture
        ...
        elements = {
            ...
            "contexts": {
                ...
                "ctx2": {"name": "ctx2", "environment": "prod"},
            },
            "components": {
                "comp1": {"name": "comp1", "priority": 1},
                "comp2": {"name": "comp2", "priority": 2},
            },
        }
        ...
        def apply_filters_mock(elements, filters):
            # Return mock results based on the filters
            result = {
                "workspaces": elements["workspaces"].copy(),
                "contexts": elements["contexts"].copy(),
                "components": elements["components"].copy(),
            }

            # Handle different filter test cases
            if len(filters) == 1:
                ...
                # Active filter
                if property_name == "active" and value is True:
                    result["workspaces"] = {"ws1": elements["workspaces"]["ws1"]}

                # Environment filter
                elif property_name == "environment" and value == "dev":
                    result["contexts"] = {"ctx1": elements["contexts"]["ctx1"]}

                # Priority filter - test new operators
                elif property_name == "priority" and operator == ">=" and value == 2:
                    result["components"] = {"comp2": elements["components"]["comp2"]}
                elif property_name == "priority" and operator == "<=" and value == 1:
                    result["components"] = {"comp1": elements["components"]["comp1"]}

            return result

        # Set up the mock filter service
        mock_filter_service.apply_filters = apply_filters_mock

        # Test with empty filters - should return original elements
        result = service.apply_filters(elements, [])
        ...
        result = service.apply_filters(elements, filters)
        assert len(result["workspaces"]) == 1
        assert "ws1" in result["workspaces"]
        assert "ws2" not in result["workspaces"]

        # Test with environment=dev filter
        filters = [{"property": "environment", "operator": "=", "value": "dev"}]
        result = service.apply_filters(elements, filters)
        assert len(result["contexts"]) == 1
        assert "ctx1" in result["contexts"]
        assert "ctx2" not in result["contexts"]

        # Test with priority>=2 filter (new operator)
        filters = [{"property": "priority", "operator": ">=", "value": 2}]
        result = service.apply_filters(elements, filters)
        assert len(result["components"]) == 1
        assert "comp2" in result["components"]
        assert "comp1" not in result["components"]

        # Test with priority<=1 filter (new operator)
        filters = [{"property": "priority", "operator": "<=", "value": 1}]
        result = service.apply_filters(elements, filters)
        assert len(result["components"]) == 1
